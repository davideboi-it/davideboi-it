# Hi, I'm Davide Boi

I have a background in Computer Science from my earlier studies. After gaining experience in different industries (including mechanics, which sharpened my technical troubleshooting and manual skills); I am now returning to my roots in the IT field with total focus and dedication.

I am a proactive learner, combining my previous technical foundation with modern certifications to become a versatile System Administrator.

### Core Technical Skills
- System Administration: Linux (Ubuntu, Debian), SSH, Bash scripting.
- IT Infrastructure: PC assembly, hardware maintenance, and troubleshooting.
- Productivity & Creative Tools: User of Microsoft 365 suite and Adobe Creative Cloud.
- Mechanics: Strong diagnostic and problem solving skills derived from technical mechanical work.

### Current Goals
- Pursuing Linux Essentials and CompTIA Server+ certifications.
- Documenting my hands-on experience in my Linux Training Lab.
- Keep improving my IT knowledge and skills.

I am driven by the challenge of fixing complex systems, whether they are made of software, hardware, or gears.
<!--
**davideboi-it/davideboi-it** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
